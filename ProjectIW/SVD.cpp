...
namespace MDC
{
	...
	{
		MDC::calcSymmetricGivensCoefficients(m.m00, m.m01, m.m11, c, s);
		const float cc = c * c;
		const float ss = s * s;
	...
	{
		MDC::calcSymmetricGivensCoefficients(m.m00, m.m02, m.m22, c, s);
		const float cc = c * c;
		const float ss = s * s;
	...
	{
		MDC::calcSymmetricGivensCoefficients(m.m11, m.m12, m.m22, c, s);
		const float cc = c * c;
		const float ss = s * s;
